...
export default function PlumBackground() {
  ...
  const { random } = Math
  const MIN_BRANCH = 30
  const len = 6 // Increased from 4 for faster growth
  ...
  useEffect(() => {
    ...
    // Styling - subtle white lines for dark theme
    ...
    const interval = 1000 / 60 // 60 FPS for faster, smoother rendering
    ...
    const frame = () => {
      ...
      prevSteps.forEach((i) => {
        if (random() < 0.7) steps.push(i) // Increased from 0.5 for faster branching
        else i()
      })
    ...
    const start = () => {
      ...
      steps = [
        // From Left side (growing right) - more initial branches
        () => step(0, randomMiddle() * height, 0),
        () => step(0, randomMiddle() * height, 0),
        () => step(0, randomMiddle() * height, 0),
        // From Right side (growing left) - more initial branches
